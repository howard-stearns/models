...
function shootDice(position, velocity) {
  if (!Entities.canRez()) {
    ...
    function tryRoll() {
      // Make sure we have known IDs so that the collision detection code will work. If not, wait.
      if (!dice.every(function (die) { return Entities.identifyEntity(die).isKnownID; })) {
	Script.setTimeout(tryRoll, 20);
	return;
      }
