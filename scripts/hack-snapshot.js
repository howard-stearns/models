...
function onClicked() {
    ...
    Script.setTimeout(function () {
        hmdType = HMD.active && 'OpenVR (Vive)';
        if (hmdType) {
            Menu.setIsOptionChecked("Desktop", true);
        }
        print('mirror, hmdType = ', hmdType);
        Menu.setIsOptionChecked("Mirror", true);  // FIXME: not mirror!
        Script.setTimeout(function () {
            print('snapping');
...
function resetButtons(path, notify) {
    if (hmdType) {
        print('switching', hmdType);
        Menu.setIsOptionChecked(hmdType, true);
    }
    Menu.setIsOptionChecked("First Person", true); // FIXME: whatever it was!
