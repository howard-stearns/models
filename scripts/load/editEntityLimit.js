...
function doFlash() {
    ...
    var elapsed = Date.now() - start, service = start - lastService;
    lastService = start;
    print(service, elapsed, elapsed / totalCreated);
}
var creator = Script.setInterval(function () {
    ...
    for (i = 0; i < numToCreate; i++) {
	    ...
	    //script: "http://howard-stearns.github.io/models/scripts/load/entityScriptEditFasterUpdate.js",
            lifetime: LIFETIME
        };
        if (isModel) {
	    ...
	    flasher = Script.setInterval(doFlash, Math.ceil((1/60) * 1000));
	    Script.setTimeout(function () { Script.clearTimeout(flasher); Script.stop(); }, LIFETIME * 1000);
        }
    }
}, SCRIPT_INTERVAL);
