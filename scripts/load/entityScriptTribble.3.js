...
var Vec3, Quat, MyAvatar, Entities, Camera, Script, AvatarList, print;
(function () {
    ...
    function checkClaim() {
        ...
        if (userData.owner === MyAvatar.sessionUUID) {
            ...
            print('claim it', userData);
            userData.owner = MyAvatar.sessionUUID;
            writeNewUserData(userData);
        } else { // Check if owner has gone away.
            var avatar = AvatarList.getAvatar(userData.owner);
            // quirk: getAvatar of missing id will answer falsey or our own avatar
            if (!avatar || (avatar === AvatarList.getAvatar(MyAvatar.sessionUUID))) {
                print('clear it', userData);
