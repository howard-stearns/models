...
function printAvatar(label) {
    print('avatar', label, Agent.sessionUUID, JSON.stringify({id: Avatar.sessionUUID, displayName: Avatar.displayName, position: Avatar.position, orientation: Avatar.orientation, skeletonModelURL: Avatar.skeletonModelURL}));
}
...
function startAgent(parameters) { // Can also be used to update.
    ...
    printAvatar('session');
    if (parameters.displayName || !Avatar.displayName) {
        Avatar.displayName = (parameters.displayName === undefined) ? Agent.sessionUUID : parameters.displayName;
    }
    ...
    ticker = Script.setInterval(function () { printAvatar('update'); }, 1000);
}
