...
function tick(elapsed) {
    print('avatar', Agent.sessionUUID, elapsed, JSON.stringify(Avatar.position));
}
var last = Date.now();
function emitUpdate() {
    var now = Date.now(), elpased = (now - last) / 1000.0;
    last = now;
    Script.update(elapsed);
}
var synthesizedUpdate = Script.setInterval(emitUpdate, 17);
Script.scriptEnding.connect(function () { Script.clearInterval(synthesizedUpdate); });
